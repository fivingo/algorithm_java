...
public class BinTree<K, V> {
    ...
    public boolean remove(K key) {
        ...
        if (p.left == null) {                           // p에는 왼쪽 자식이 없음
            ...
            if (p == root) {
                ...
                parent.right = p.left;                  // 부모의 오른족 포인터가 왼쪽 자식을 가리킴
            }
        } else {
            parent = p;
            ...
            p.data = left.data;                         // left의 데이터를 n으로 옮김
    ...
    private void printSubTree(Node node) {
        if (node != null) {
            ...
            printSubTree(node.right);                   // 오른족 서브 트리를 키 값의 오름차순으로 출력
        }
    }
    ...
    private void printSubTreeR(Node node) {
        if (node != null) {
            ...
            System.out.println(node.key + " " + node.data);     // node를 출력
            printSubTree(node.left);                    // 오른족 서브 트리를 키 값의 오름차순으로 출력
        }
    }

    // 모든 노드를 키 캆의 오름차순으로 출력
    public void print() {
        printSubTree(root);
    }

    // 모든 노드를 키 캆의 내름차순으로 출력
