...
public class OpenHash<K, V> {
    ...
    private Buchet<K, V>[] table;           // 해시 테이블

    // 생성자
    public OpenHash(int size) {
        try {
            table = new Buchet[size];

            for(int i = 0; i < size; i ++) {
                table[i] = new Buchet<K, V>();
            }
    ...
    // 키 값 key를 갖는 버킷의 검색
    public Buchet<K, V> searchNode(K key) {
        int hash = hashValue(key);          // 검색할 데이터의 해시 값
        Buchet<K, V> p = table[hash];       // 선택 버킷
    ...
    public int remove(K key) {
        Buchet<K, V> p = searchNode(key);   // 선택 버킷
