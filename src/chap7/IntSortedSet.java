...
public class IntSortedSet {
    ...
    public boolean add(int n) {
        ...
        if (num >= max || (idx = indexOf(n)) >= 0) {        // 가득 찼거나 이미 n이 존재
            return false;
        } else {
            idx = -(idx + 1);
            num++;
    ...
    // 집합 s와 합집합
    public boolean canCombine(IntSortedSet s) {
        boolean done = false;
    ...
    // 집합 s와 교집합
    public boolean canIntersection(IntSortedSet s) {
        boolean done = false;
    ...
    // 집합 s와 차집합
    public boolean canDifference(IntSortedSet s) {
        boolean done = false;
